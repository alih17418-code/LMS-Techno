...
import { ArrowLeft, Printer, Plus, Trash2, AlertCircle, User, Phone, MapPin, BookOpen, ClipboardList, TrendingUp } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Attendance log, earnings, and payment history.</p>
        </div>
      </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className={cn("inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border shrink-0",
                      instructor.status === "active" ? "text-green-700 bg-green-50 border-green-200 dark:text-green-400 dark:bg-green-950/40 dark:border-green-800" : "text-slate-500 bg-slate-50 border-slate-200 dark:bg-slate-900/40 dark:border-slate-700")}>
                      {instructor.status}
                    </span>
                    <span className={cn("inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border shrink-0",
                      paymentModel === "per_lecture" ? "text-purple-700 bg-purple-50 border-purple-200 dark:text-purple-400 dark:bg-purple-950/40 dark:border-purple-800"
                      : paymentModel === "commission" ? "text-amber-700 bg-amber-50 border-amber-200 dark:text-amber-400 dark:bg-amber-950/40 dark:border-amber-800"
                  ...
                  <div className="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
                    ...
                    {instructor.specialization && <span>Specialization: <strong className="text-foreground">{instructor.specialization}</strong></span>}
                  ...
                  <div className="mt-2 text-sm text-muted-foreground">
                    ...
                    {paymentModel === "salary" && <>Monthly Salary: <strong className="text-foreground font-mono">{formatCurrency(instructor.monthlySalary)}</strong></>}
                      ...
                      <tbody>
                        {instructor.payments.map((p) => (
                          <tr key={p.id} className="border-b border-border hover:bg-muted/20 transition-colors">
                            ...
                            <td className="px-4 py-3 text-right font-mono font-semibold text-green-600 dark:text-green-500">{formatCurrency(p.amountPaid)}</td>
                        ...
                        <tr className="bg-muted/20 font-semibold">
                          ...
                          <td className="px-4 py-3 text-right font-mono text-green-600 dark:text-green-500">{formatCurrency(totalPaid)}</td>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-3">
                <div>
                  ...
                  <CardDescription>
                    {paymentModel === "per_lecture"
                      ? `${totalLectures} lectures × ${formatCurrency(lectureRate)}/lecture = ${formatCurrency(totalEarned)} earned`
                      : "Track classes delivered by this instructor."}
                  </CardDescription>
                </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-border bg-muted/30">
                          ...
                          {paymentModel === "per_lecture" && <th className="text-right px-4 py-3 font-medium text-muted-foreground">Earned</th>}
                      ...
                      <tbody>
                        {attendList.map((a) => (
                          <tr key={a.id} className="border-b border-border hover:bg-muted/20 transition-colors">
                            ...
                            {paymentModel === "per_lecture" && (
                              <td className="px-4 py-3 text-right font-mono text-green-600">{formatCurrency(a.lectureCount * lectureRate)}</td>
                            )}
                            <td className="px-4 py-3">
                              <span className={cn("inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border",
                                a.status === "present" ? "text-green-700 bg-green-50 border-green-200 dark:text-green-400 dark:bg-green-950/40 dark:border-green-800"
                                : a.status === "absent" ? "text-red-700 bg-red-50 border-red-200 dark:text-red-400 dark:bg-red-950/40 dark:border-red-800"
                        ...
                        <tr className="bg-muted/20 font-semibold">
                          ...
                          {paymentModel === "per_lecture" && (
                            <td className="px-4 py-3 text-right font-mono text-green-600">
                              {formatCurrency(attendList.reduce((s, a) => s + a.lectureCount * lectureRate, 0))}
                            </td>
                          )}
                          <td colSpan={2}></td>
                        </tr>
          ...
          <div className="grid grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-1">
              ...
              {instructorClasses.length > 0 ? (
                <Select
                  value={attendForm.classId || "none"}
                  onValueChange={(v) => {
                    if (v === "none") {
                      setAttendField("classId", "");
                      setAttendField("className", "");
                    } else {
                      const cls = instructorClasses.find(c => String(c.id) === v);
                      setAttendField("classId", v);
                      setAttendField("className", cls?.className ?? "");
                    }
                  }}
                >
                  ...
                  <SelectContent>
                    ...
                    {instructorClasses.map(c => (
                      <SelectItem key={c.id} value={String(c.id)}>
                        {c.className} <span className="text-muted-foreground">({c.courseName})</span>
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
      ...
      <AlertDialog open={!!deleteAttendTarget} onOpenChange={(o) => !o && setDeleteAttendTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Delete attendance for <strong>{deleteAttendTarget?.attendanceDate}</strong>? This will also reduce the calculated earnings.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
